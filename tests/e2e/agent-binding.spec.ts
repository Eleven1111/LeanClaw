...
test('Agent 身份贯穿 Task 快照、列表、看板与定时计划', async () => {
  ...
  await window.getByPlaceholder('计划名称').fill('Agent 定时计划')
  await window.getByRole('button', { name: '保存计划' }).click()
  const schedule = window.locator('.schedule-card', { hasText: 'Agent 定时计划' })
  await expect(schedule).toContainText('Agent：Snapshot Agent V2')
  ...
  await expect(window.locator('.schedules-section .error')).toContainText('Agent 已停用')
  ...
  await schedule.getByRole('button', { name: '改绑 Agent' }).click()
  await window.getByLabel('改绑 Agent 定时计划 的 Agent').selectOption(replacementAgentId)
  await launched.app.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0].setSize(1440, 900)
  })
  await window.waitForTimeout(250)
  await window.screenshot({ path: '.omx/state/agent-binding/schedule-rebind.png' })
  await window.getByRole('button', { name: '保存改绑' }).click()
  await expect(schedule).toContainText('Agent：Replacement Agent')
  await schedule.getByRole('button', { name: '启用' }).click()
